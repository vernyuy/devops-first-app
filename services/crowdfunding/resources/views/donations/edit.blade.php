        ...
        <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto h-[80vh] lg:py-0">
            <div class="w-full bg-white rounded-lg shadow md:mt-0 sm:max-w-md xl:p-0">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form class="space-y-4 md:space-y-6" action="{{ route('donations.update',$donation->id) }}" method="POST">
                        @csrf
                        @method('PUT')

                        <div>
                            <label for="reason" class="block mb-2 text-sm font-medium text-gray-900 ">Reason</label>
                            <textarea type="text" name="reason" placeholder="Enter your reason" class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 " required>{{$donation->reason}}</textarea>
                        </div>
